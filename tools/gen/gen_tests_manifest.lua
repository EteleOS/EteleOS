...
for _, category in ipairs(TEST_CATEGORIES) do
    ...
    if os.isdir(catdir) then
        ...
        print(string.format("os-gen-tests: tests/%s not found, skipping", category))
    end
end

print(string.format("os-gen-tests: %d dirs with a Makefile, %d script-only (arch=%s)",
...
out[#out + 1] = ""
out[#out + 1] = "OS_TESTS_MANIFEST = " .. serialize(MANIFEST)
out[#out + 1] = ""
...
f:close()
print(string.format("os-gen-tests: manifest written to %s", outpath))
