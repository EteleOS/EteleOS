--[[
PeteleOS: tools/gen/gen_userland_manifest.lua, time write: 2026/07/26
...
(also wired up as the `os-regen-userland` task in tools/tasks.lua, so
`xmake os-regen-userland` works too once a project is configured).
...
-- Discovery walk -- same shape as the original os_walk_dir/
-- os_category, but RECORDS build units into `UNITS` instead of calling
...
local function walk_dir(dir, relpath, target_prefix, static)
    ...
    if info.found and #info.subdirs > 0 and not info.prog and #info.progs == 0 then
        ...
        for _, sub in ipairs(info.subdirs) do
            ...
            if os.isdir(subdir) then
                ...
                print(string.format("os-gen: %s: SUBDIR entry '%s' does not exist, skipping",
                      relpath, sub))
            end
        end
        return
    ...
    if info.found and #info.progs > 0 and not info.prog then
        ...
        for _, name in ipairs(info.progs) do
            ...
            if src then
                ...
                print(string.format("os-gen: %s: PROGS entry '%s' -- no %s.c found, skipping",
                      relpath, name, name))
            end
        end
        return
    ...
    -- the original os_program()'s own early-return for a genuinely
...
local function walk_category(category, label, static)
    ...
    if not os.isdir(catdir) then
        print(string.format("os-gen: userland/%s not found, skipping", category))
        return
    end
...
local function serialize(value, indent)
    ...
    elseif t == "table" then
        ...
        raise("os-gen: cannot serialize value of type " .. t)
    end
end
...
out[#out + 1] = "     (or: xmake os-regen-userland, once a project is configured) ]]"
out[#out + 1] = ""
out[#out + 1] = "OS_USERLAND_MANIFEST = " .. serialize(UNITS)
out[#out + 1] = ""
...
print(string.format(
    "os-gen: userland manifest written to %s\n" ..
