...
(also wired up as the `peteleos-regen-userland` task in tools/tasks.lua, so
`xmake peteleos-regen-userland` works too once a project is configured).
    ...
    if info.found and #info.subdirs > 0 and not info.prog and #info.progs == 0 then
        ...
        for _, sub in ipairs(info.subdirs) do
            ...
            if os.isdir(subdir) then
                ...
                print(string.format("peteleos-gen: %s: SUBDIR entry '%s' does not exist, skipping",
                      relpath, sub))
            end
        end
        return
    ...
    if info.found and #info.progs > 0 and not info.prog then
        ...
        for _, name in ipairs(info.progs) do
            ...
            if src then
                ...
                print(string.format("peteleos-gen: %s: PROGS entry '%s' -- no %s.c found, skipping",
                      relpath, name, name))
            end
        end
        return
...
local function walk_category(category, label, static)
    ...
    if not os.isdir(catdir) then
        print(string.format("peteleos-gen: userland/%s not found, skipping", category))
        return
    end
...
local function serialize(value, indent)
    ...
    elseif t == "table" then
        ...
        raise("peteleos-gen: cannot serialize value of type " .. t)
    end
end
...
out[#out + 1] = "     (or: xmake peteleos-regen-userland, once a project is configured) ]]"
out[#out + 1] = ""
...
print(string.format(
    "peteleos-gen: userland manifest written to %s\n" ..
