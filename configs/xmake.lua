...
target("os-configs")
    ...
    after_install(function (target)
        ...
        for _, category in ipairs({"logging", "network", "security", "system"}) do
            ...
            if n == 0 then
                wprint("os-configs: configs/%s not found or empty, skipping", category)
            end
            total = total + n
        end
        ...
        if os.isdir(archdir) then
            ...
            wprint("os-configs: configs/etc/etc.%s not found, skipping "
                   .. "architecture-specific /etc files (login.conf, sysctl.conf, "
                   .. "disktab, ttys, ...)", arch)
        end

        cprint("${green}os-configs${clear}: installed %d files to %s (arch=%s)",
               total, etcdir, arch)
    end)
target_end()
